...
@Component({
    ...
    providers: [
        { provide: MatFormFieldControl, useExisting: LanguageSelectComponent }
    ]
})
...
export class LanguageSelectComponent implements OnInit, ControlValueAccessor,
    ...
    focused = false;
    controlType = 'app-language-select';
    ...
    autocompleteLanguages: Observable<Language[]>;
    ...
    writeValue(value) {
        ...
        if (this.mode === 'autocomplete') {
            if (this.value) {
                this.languageService.getLanguage({ id: this.value })
                    .subscribe(language => this.autocompleteControl.setValue(language));
            } else {
    ...
    ngOnInit(): void {
        if (this.mode === 'select') {
            this.languageService.getLanguages({}).subscribe(languages => this.languages = languages.pageItems);
        } else if (this.mode === 'autocomplete') {
            this.autocompleteLanguages = this.autocompleteControl.valueChanges
                .pipe(
                    map(value => {
                        if (value === '') {
