...
@Component({
  ...
  providers: [
    { provide: MatFormFieldControl, useExisting: PageSelectComponent }
  ]
})
...
export class PageSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-page-select';
  id = `page-select-${PageSelectComponent.nextId++}`;
  describedBy = '';
  ...
  autocompletePages: Observable<Page[]>;
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.pageService.getPage({ id: this.value })
          .subscribe(page => this.autocompleteControl.setValue(page));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      this.pageService.getPages({}).subscribe(pages => this.pages = pages.pageItems);
    } else if (this.mode === 'autocomplete') {
      this.autocompletePages = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
