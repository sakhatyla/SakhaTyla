...
@Component({
  ...
  providers: [
    { provide: MatFormFieldControl, useExisting: WidgetSelectComponent }
  ]
})

export class WidgetSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-widget-select';
  id = `widget-select-${WidgetSelectComponent.nextId++}`;
  describedBy = '';
  ...
  autocompleteWidgets: Observable<Widget[]>;
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.widgetService.getWidget({ id: this.value })
          .subscribe(widget => this.autocompleteControl.setValue(widget));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      this.widgetService.getWidgets({}).subscribe(widgets => this.widgets = widgets.pageItems);
    } else if (this.mode === 'autocomplete') {
      this.autocompleteWidgets = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
