...
@Component({
  selector: 'app-worker-info-select',
  templateUrl: './worker-info-select.component.html',
  providers: [
    { provide: MatFormFieldControl, useExisting: WorkerInfoSelectComponent }
  ]
})

export class WorkerInfoSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-worker-info-select';
  id = `worker-info-select-${WorkerInfoSelectComponent.nextId++}`;
  describedBy = '';
  ...
  autocompleteWorkerInfos: Observable<WorkerInfo[]>;
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.workerInfoService.getWorkerInfo({ id: this.value })
          .subscribe(workerInfo => this.autocompleteControl.setValue(workerInfo));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      this.workerInfoService.getWorkerInfos({}).subscribe(workerInfos => this.workerInfos = workerInfos.pageItems);
    } else if (this.mode === 'autocomplete') {
      this.autocompleteWorkerInfos = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
          ...
          mergeMap(val => {
            if (val.length !== 0) {
              return this.workerInfoService.getWorkerInfos({
                filter: { text: val }
              }).pipe(map(res => res.pageItems));
            } else {
              return of(<WorkerInfo[]>[]);
            }
          }));
    }
  }
  ...
  getDisplay(workerInfo: WorkerInfo) {
    return workerInfo ? workerInfo.name : '';
  }
}
