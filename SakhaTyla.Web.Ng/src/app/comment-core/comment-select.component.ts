...
@Component({
  selector: 'app-comment-select',
  templateUrl: './comment-select.component.html',
  providers: [
    { provide: MatFormFieldControl, useExisting: CommentSelectComponent }
  ]
})

export class CommentSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-comment-select';
  id = `comment-select-${CommentSelectComponent.nextId++}`;
  describedBy = '';
  ...
  autocompleteComments: Observable<Comment[]>;
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.commentService.getComment({ id: this.value })
          .subscribe(comment => this.autocompleteControl.setValue(comment));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      this.commentService.getComments({}).subscribe(comments => this.comments = comments.pageItems);
    } else if (this.mode === 'autocomplete') {
      this.autocompleteComments = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
