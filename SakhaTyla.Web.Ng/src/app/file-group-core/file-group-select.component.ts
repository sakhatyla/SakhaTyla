...
export class FileGroupSelectComponent implements OnInit, ControlValueAccessor,
  ...
  autocompleteFileGroups: Observable<FileGroup[]>;
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      ...
      this.autocompleteFileGroups = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
          ...
          mergeMap(val => {
            if (val.length !== 0) {
              return this.fileGroupService.getFileGroups({
                filter: { text: val }
  ...
  getDisplay(fileGroup: FileGroup) {
