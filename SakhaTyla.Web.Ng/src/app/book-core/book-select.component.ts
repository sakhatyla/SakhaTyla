...
@Component({
  ...
  providers: [
    { provide: MatFormFieldControl, useExisting: BookSelectComponent }
  ]
})

export class BookSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-book-select';
  id = `book-select-${BookSelectComponent.nextId++}`;
  describedBy = '';
  ...
  autocompleteBooks: Observable<Book[]>;
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.bookService.getBook({ id: this.value })
          .subscribe(book => this.autocompleteControl.setValue(book));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      this.bookService.getBooks({}).subscribe(books => this.books = books.pageItems);
    } else if (this.mode === 'autocomplete') {
      this.autocompleteBooks = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
