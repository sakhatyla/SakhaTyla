...
@Component({
  ...
  providers: [
    { provide: MatFormFieldControl, useExisting: TagSelectComponent }
  ]
})
...
export class TagSelectComponent implements OnInit, ControlValueAccessor,
  ...
  focused = false;
  controlType = 'app-tag-select';
  id = `tag-select-${TagSelectComponent.nextId++}`;
  describedBy = '';
  ...
  writeValue(value) {
    ...
    if (this.mode === 'autocomplete') {
      if (this.value) {
        this.tagService.getTag({ id: this.value })
          .subscribe(tag => this.autocompleteControl.setValue(tag));
      } else {
  ...
  ngOnInit(): void {
    if (this.mode === 'select') {
      ...
      this.autocompleteTags = this.autocompleteControl.valueChanges
        .pipe(
          map(value => {
            if (value === '') {
